...
const clientCreds =
  'eyJhbGciOiJSUzI1NiIsImtpZCI6IjEiLCJ0eXAiOiJKV1QifQ..gFW7HXL-CNCg4KUuAC79Kq43ptTpSpDRa_naTlh9kDmU5aWGLYy6LusvJnBGFA60xBdas0Wn37i58-7r2R-mdg43uU1aIfLn_eIL5CChTYkVFltwY5G8LPSGREnwe1auxtrpnYJYGB5IK-thehnlENZjWiJpFhj_tJkLp2JKy2VcXtCYNaauS7kfgXDr8JZMMIaV7SyMNdtbkYg6eiOEWR0pZxiVVYfym96TPZGctQjTDFV8Yh0Xa7Nqrf7_Fz-hPgfPBPc93EfdVZ0nQqIfHcj8stfQl6u3j5b0xB5SYedvjt-ajoURaZohgZu085TLAAut_abWHhrPovbWrAQzGw';
...
export const getAccountData = async (accountId: String, info: any) => {
  // console.log('info::', info.fieldNodes[0].selectionSet.selections);
  const isUserRequested = info.fieldNodes[0].selectionSet.selections.some(
    (selection: { name: { value: string } }) => selection.name.value === 'users'
  );
...
export const getAccountDataByAccountId = async (accountId: String) => {
  // Get account details
  let response = await axios.get(
    config.service.profiles.baseUrl +
      config.service.profiles.endpoints.accountDetails.replace(
...
export const getAddressessLinkedToAccount = async (accountId: String) => {
  // Get addresses linked to an account
  try {
    let addressesResponse = await axios.post(
      config.service.profiles.baseUrl +
...
export const getUsersLinkedToAccount = async (accountId: String, info: any) => {
  let usersResponse = await axios.post(
    config.service.profiles.baseUrl + config.service.profiles.endpoints.users,
    {
      relations: [
